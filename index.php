...
function utilFetchBoth_Db($db, $approvedWhereDaily, $userId, $userRole, $dateFrom, $dateTo, $agg = 'SUM', $fallbackTariffs = null) {
    ...
    try {
        ...
        // Strategy dedup: Subquery cari MAX(id) per (tgl, engineer) karena id PK auto-inc selalu konsisten urutan waktu
        $sqlD = "SELECT
        ...
        if ($isSum && ((float)($d['elec'] ?? 0) < 0.00001 || (float)($d['water'] ?? 0) < 0.00001)) {
            ...
            if ($needElec || $needWater) {
                ...
                // ✅ Fallback juga DEDUP MAX(id) per (tgl, engineer), biar tidak dobel perhitungan jika user isi berkali-kali sehari
...
/** Cek apakah baris utility merge punya nilai konsumsi */
function utilRowHasData(array $row): bool {
    return ((float)($row['elec'] ?? 0) + (float)($row['water'] ?? 0) + (float)($row['gas'] ?? 0) + (float)($row['fuel'] ?? 0)) > 0.00001;
}

/** Cari tanggal terakhir (<= asOfDate) yang punya data utility — sinkron dengan energy.php detail logs */
function utilFindLatestDateWithData($db, $asOfDate, $dailyWhere, $userId, $userRole, $lookbackDays = 21) {
...
/* Manager/Supervisor/Admin: tampilkan juga log pending (sering belum di-approve tapi sudah diisi engineer) */
$_todayUtilWhere = "status='approved' $statusWhere";
...
if (!utilRowHasData($todayBoth)) {
    ...
    if ($_latUtil) {
        ...
        $utilDisplayDate = $_latUtil;
    }
    unset($_latUtil);
}
unset($_todayUtilWhere);
...
try {
    $masterWhere = "WHERE am.status_default = 'progress'";
    $masterParams = [];
    if ($userRole === 'engineer') {
        // Engineer tidak punya filter master, tapi show semua biar tau daftar progress
    }
    $masterRows = $db->fetchAll("SELECT am.id as mid, am.division, am.activity_name, am.sort_order, am.created_at,
    ...
    // âœ… Fallback nama engineer untuk progress widget master (belum ditugaskan):
    //    Prioritas 1: nama user pembuat master (created_by_name dari JOIN users)
    //    Prioritas 2: nama user login (jika created_by null)
    //    Prioritas 3: label default
    foreach ($masterRows as $m) {
        ...
        $idxDaily++;
        $_engName = !empty($m['created_by_name']) ? (string)$m['created_by_name']
                  : (!empty($user['name']) ? (string)$user['name'] : '- (Belum ditugaskan)');
        $progressActivities[] = [
...
try {
    ...
    foreach ($_tmpMastersAct as $_m) {
        ...
        $_engLabel = !empty($_m['created_by_name']) ? (string)$_m['created_by_name']
                  : (!empty($user['name']) ? (string)$user['name'] : '- (Master Activity)');
        $actsGRP[$dv][] = ['title' => $title, 'status' => ($st === 'complete' ? 'complete' : 'progress'), 'date' => (string)($_m['created_at'] ?? ''), 'eng' => $_engLabel];
        $_existingTitleAct[$dv][$key] = true;
    }
    unset($_tmpMastersAct, $_existingTitleAct, $dv, $_m, $title, $key, $st, $_engLabel);
} catch (Throwable $e) {}

...
<div class="page-shell page-shell--7xl pb-20 md:pb-10">
    ...
    <!-- STATISTIC 4 CARDS (DOMINAN PUTIH!) -->
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm hover:shadow-md transition">
            <div class="flex items-center justify-between mb-2">
                ...
                <span class="w-9 h-9 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-500"><i class="fas fa-users"></i></span>
            </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm hover:shadow-md transition">
            <div class="flex items-center justify-between mb-2">
                <span class="text-[11px] font-bold uppercase tracking-[0.15em] text-slate-500">Manager</span>
                <span class="w-9 h-9 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-purple-600"><i class="fas fa-crown"></i></span>
            </div>
            <p class="font-display text-3xl font-black text-primary leading-none"><?= $cntMgr ?></p>
            <p class="text-[11px] text-slate-500 mt-1">Akses penuh semua fitur</p>
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm hover:shadow-md transition">
            <div class="flex items-center justify-between mb-2">
                <span class="text-[11px] font-bold uppercase tracking-[0.15em] text-slate-500">Supervisor</span>
                <span class="w-9 h-9 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-blue-600"><i class="fas fa-user-shield"></i></span>
            </div>
            <p class="font-display text-3xl font-black text-primary leading-none"><?= $cntSpv ?></p>
            <p class="text-[11px] text-slate-500 mt-1">Review &amp; Approval Daily Log</p>
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm hover:shadow-md transition">
            <div class="flex items-center justify-between mb-2">
                <span class="text-[11px] font-bold uppercase tracking-[0.15em] text-slate-500">Staff Engineer</span>
                <span class="w-9 h-9 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-emerald-600"><i class="fas fa-hard-hat"></i></span>
            </div>
            <p class="font-display text-3xl font-black text-primary leading-none"><?= $cntEng ?></p>
            <p class="text-[11px] text-slate-500 mt-1">Isi Daily Log &amp; Order Request</p>
        </div>
    </div>

    <!-- TABLE FILTER & SEARCH (PUTIH BERSIH) -->
            ...
            <table class="w-full text-sm min-w-[1000px]">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors group">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    <p class="font-semibold text-primary">
                                        <?= cleanInput($u['name']) ?>
                                        <?php if ($isMe): ?>
                                            <span class="ml-1 inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-100 border border-slate-200 text-slate-700 text-[9px] font-black uppercase tracking-wider"><i class="fas fa-hand-sparkles text-[8px]"></i> ANDA</span>
                                        <?php endif; ?>
                                    </p>
                        ...
                        <td class="px-4 sm:px-5 py-4 text-center align-top"><span class="inline-flex items-center justify-center px-3 py-1 rounded-full bg-slate-100 border border-slate-200 text-slate-700 font-bold min-w-[52px]"><?= (int)($u['total_orders'] ?? 0) ?></span></td>
                        <td class="px-4 sm:px-5 py-4 align-top">
                            <?php if ($lastLogin): ?>
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 border border-slate-200 text-slate-700 text-[11px] font-bold">
                        ...
                        <td class="px-4 sm:px-5 py-4 pr-5 align-top">
                            <div class="flex gap-2 justify-end flex-wrap items-center">
                                ...
                                <button type="button" onclick='confirmDeleteUser(<?= (int)$u['id'] ?>, <?= json_encode((string)$u['name'], JSON_UNESCAPED_UNICODE) ?>, <?= (int)$isMe ? 'true' : 'false' ?>)'
                                        class="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold bg-white text-slate-600 border border-slate-200 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 hover:-translate-y-0.5 transition-all <?= $isMe ? 'opacity-50 cursor-not-allowed pointer-events-none' : '' ?>"
                    ...
                    <tr><td colspan="7" class="px-4 sm:px-5 py-20 text-center text-secondary">
                        ...
                        <?php else: ?>
                            <button type="button" onclick="openUserModal(0)" class="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-card bg-slate-900 text-white text-xs font-bold shadow-md hover:bg-slate-800 hover:shadow-lg transition">
...
<div id="modal-user" class="fixed inset-0 z-[99999] hidden items-center justify-center p-4 animate-fade-in-modal"
    ...
    <div class="relative w-full max-w-3xl max-h-[93vh] bg-white rounded-3xl shadow-[0_30px_100px_-20px_rgba(30,41,59,0.45)] overflow-hidden flex flex-col animate-slide-up-modal border-2 border-slate-200">
        <!-- HEADER MODAL (PUTIH!) -->
        <div class="bg-white p-5 sm:p-6 text-primary relative border-b-2 border-slate-200">
            <div class="flex items-start justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-[10px] sm:text-[11px] font-black uppercase tracking-[0.25em] text-slate-500 mb-1.5">Form Pengelolaan Akun</p>
                        <h4 id="modalUserTitle" class="font-display text-2xl sm:text-3xl font-black tracking-wide leading-tight text-primary">Tambah Akun Baru</h4>
                        <div class="flex items-center gap-3 mt-2">
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 border border-slate-200 text-slate-600 text-xs font-bold">
                                <i class="fas fa-shield-halved text-slate-500"></i> Hanya Manager yang bisa akses
                            </span>
                        </div>
                    </div>
                </div>
        ...
        <form method="POST" id="formUser" class="flex-1 overflow-y-auto">
            ...
            <div class="p-5 sm:p-6 space-y-5 bg-white">
                <!-- PILIH ROLE (PALING ATAS - PUTIH BERSIH!) -->
                <div class="p-4 sm:p-5 rounded-2xl bg-white border-2 border-dashed border-slate-200 animate-fade-in">
                    <p class="text-[11px] font-black uppercase tracking-[0.2em] text-slate-600 mb-2 flex items-center gap-1.5">
                        <i class="fas fa-key text-slate-500"></i> 1. Pilih LEVEL / ROLE AKUN
                    </p>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3" id="rolePicker">
                        <label class="cursor-pointer role-card" data-role="manager">
                            ...
                            <div class="rounded-xl border-2 border-slate-200 bg-white p-3 peer-checked:border-slate-900 peer-checked:bg-slate-50 hover:border-slate-400 hover:-translate-y-0.5 transition-all shadow-sm">
                                <div class="flex items-center gap-3">
                                    <span class="w-11 h-11 rounded-xl bg-white border-2 border-purple-200 text-purple-600 flex items-center justify-center"><i class="fas fa-crown"></i></span>
                                    <div class="min-w-0">
                        ...
                        <label class="cursor-pointer role-card" data-role="supervisor">
                            ...
                            <div class="rounded-xl border-2 border-slate-200 bg-white p-3 peer-checked:border-slate-900 peer-checked:bg-slate-50 hover:border-slate-400 hover:-translate-y-0.5 transition-all shadow-sm">
                                <div class="flex items-center gap-3">
                                    <span class="w-11 h-11 rounded-xl bg-white border-2 border-blue-200 text-blue-600 flex items-center justify-center"><i class="fas fa-user-shield"></i></span>
                                    <div class="min-w-0">
                        ...
                        <label class="cursor-pointer role-card" data-role="engineer">
                            ...
                            <div class="rounded-xl border-2 border-slate-200 bg-white p-3 peer-checked:border-slate-900 peer-checked:bg-slate-50 hover:border-slate-400 hover:-translate-y-0.5 transition-all shadow-sm">
                                <div class="flex items-center gap-3">
                                    <span class="w-11 h-11 rounded-xl bg-white border-2 border-emerald-200 text-emerald-600 flex items-center justify-center"><i class="fas fa-hard-hat"></i></span>
                                    <div class="min-w-0">
                ...
                <div class="p-4 sm:p-5 rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <p class="text-[11px] font-black uppercase tracking-[0.2em] text-slate-600 mb-3 flex items-center gap-1.5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="md:col-span-2">
                            <label class="block text-sm font-semibold text-primary mb-2">
                                <i class="fas fa-signature mr-1.5 text-slate-500"></i>Nama Lengkap <span class="text-red-500">*</span>
                            </label>
                            <div class="relative">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-primary mb-2">
                                <i class="fas fa-envelope mr-1.5 text-slate-500"></i>Email <span class="text-red-500">*</span> <span class="text-[10px] text-slate-400 font-normal">(UNIQUE / tidak boleh sama)</span>
                            </label>
                            <div class="relative">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-primary mb-2">
                                <i class="fas fa-phone mr-1.5 text-slate-500"></i>No. Handphone / WhatsApp
                            </label>
                            <div class="relative">
                        ...
                        <div class="md:col-span-2">
                            <label class="block text-sm font-semibold text-primary mb-2">
                                <i class="fas fa-briefcase mr-1.5 text-slate-500"></i>Jabatan / Posisi
                            </label>
                            <div class="relative">
                ...
                <div class="p-4 sm:p-5 rounded-2xl border border-slate-200 bg-white shadow-sm" id="pwdWrap">
                    <p class="text-[11px] font-black uppercase tracking-[0.2em] text-slate-600 mb-3 flex items-center gap-1.5">
                        ...
                        <span id="pwdHint" class="ml-auto text-[10px] font-bold text-slate-600 uppercase tracking-wider">* WAJIB diisi untuk akun BARU</span>
                    </p>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="password" id="pwd" name="password" autocomplete="new-password" minlength="6" placeholder="••••••••"
                                    class="w-full pl-11 pr-11 py-3 rounded-card border border-border bg-muted/50 text-primary placeholder-secondary/60 focus:outline-none focus:border-slate-600 focus:ring-2 focus:ring-slate-200 focus:bg-surface transition-all">

...
if (isset($_GET['export']) && (string)$_GET['export'] === '1') {
    ...
    $fname = 'Job-Daily-Trip-' . date('Ymd-His') . '.csv';
    ...
    $out = [];
    $out[] = ['JOB DAILY TRIP TRANSPORT - The St. Regis Bali Engineering Dept','','','','','','','','','','','','','','','','','','','','',''];
    $out[] = ['Preset Filter: '.$plLabel, 'Periode: '.$from.' s/d '.$to, 'Search: '.($search ?: '-'), 'Export: '.date('d M Y H:i:s'),'','','','','','','','','','','','','','','','','',''];
    $out[] = ['','','','','','','','','','','','','','','','','','','','','',''];
    $out[] = ['=== REKAP PERIODE ===','','','','','','','','','','','','','','','','','','','','',''];
    $out[] = ['Total Trip (Unique)',$statTrip.' trip','Total Jarak (KM)',number_format($statKM,1,',','.').' KM','','','','','','','','','','','','','','','','','',''];
    $out[] = ['Harga Jual Customer (Rp)','Rp '.number_format($statJual,0,',','.'),'Harga Beli Vendor (Rp)','Rp '.number_format($statVendor,0,',','.'),'Laba Kotor (Rp)','Rp '.number_format($statLaba,0,',','.'),'','','','','','','','','','','','','','','',''];
    $out[] = ['','','','','','','','','','','','','','','','','','','','','',''];
    $out[] = ['No','ID','NO. TRIP','TANGGAL','CUSTOMER','RUTE ASAL','RUTE TUJUAN','PLAT NOMOR','UNIT','SOPIR 1','SOPIR 2','TRIP (x)','KM AWAL','KM AKHIR','TOTAL KM','HARGA JUAL (Rp)','HARGA BELI VENDOR (Rp)','FEE DRIVER (Rp)','LABA / RUGI (Rp)','STATUS TRIP','STATUS BAYAR','CATATAN'];
    $n = 0;
    foreach ($rows as $r) { $n++;
        ...
        $out[] = [
            ...
            strtoupper((string)($r['plat_no'] ?? '')), (string)($r['vehicle_name'] ?? ''), (string)($r['driver_name'] ?? ''), (string)($r['driver2_name'] ?? ''),
            (int)($r['trip_count'] ?? 1), number_format((float)($r['km_start'] ?? 0),1,',','.'),
            number_format((float)($r['km_end'] ?? 0),1,',','.'), number_format((float)($r['total_km'] ?? 0),1,',','.'),
            number_format((float)($r['price_sell'] ?? 0),0,',','.'), number_format((float)($r['price_vendor'] ?? 0),0,',','.'),
            number_format((float)($r['fee_driver'] ?? 0),0,',','.'), number_format($laba,0,',','.'),
            $st, $pay, preg_replace('/\s+/',' ',(string)($r['notes'] ?? ''))
        ];
    }
    foreach ($out as $rr) { echo implode(';', array_map('_csv3', $rr)) . "\r\n"; }
    exit;
}
